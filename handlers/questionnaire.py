...
class SecondInterviewInfoStates(StatesGroup):
       ...
       waiting_for_4_question_s = State()
       waiting_for_5_question_s = State()

class CancelStates(StatesGroup):
       waiting_for_cancel = State()


class VacancyAndTestStates(StatesGroup):
       waiting_for_vacancy_test = State()
...
async def process_hh(message: types.Message, state: FSMContext):
    await state.set_state(CancelStates())
    await state.clear()
...
async def interview(message: types.Message, state: FSMContext, title=None):
    if title is None:
        data = await state.get_data()
        print('data: {} {} {}'.format(data, data.keys(), await state.get_state()))
        title_of_vacancy = data['chosen_vacancy']
    else:
        title_of_vacancy = title
    global vacancy
    ...
    questions_for_second_interview = vacancy.interview_questions
    if questions_for_second_interview:
        print(f'\n\n!!!!\n\nquestions_for_second_interview: {questions_for_second_interview}')
        ...
        amount_questions = len(json.loads(questions_of_second_interview).keys())
        await state.set_state(SecondInterviewInfoStates.waiting_for_1_question_s)
        # await message.answer(f"Прекрасно! Осталось совсем немного!")
        await message.reply(f"{json.loads(questions_of_second_interview).get('Вопрос 1')}", reply_markup=cancel)
    else:
        await end_of_interview(message, state)
...
async def collect_responses_interviews_info(message: types.Message, state: FSMContext):
    ...
    #     else:
    #         await end_of_interview(message, state)
    if await state.get_state() == SecondInterviewInfoStates.waiting_for_1_question_s:
        await state.update_data(waiting_for_1_question_s=message.text)
        if amount_questions > 1:
            await state.set_state(SecondInterviewInfoStates.waiting_for_2_question_s)
            await message.reply(f"{json.loads(questions_of_second_interview).get('Вопрос 2')}", reply_markup=cancel)
        else:
             await end_of_interview(message, state, one=1)
        ...
        if amount_questions > 2:
            await state.set_state(SecondInterviewInfoStates.waiting_for_3_question_s)
            await message.reply(f"{json.loads(questions_of_second_interview).get('Вопрос 3')}", reply_markup=cancel)
        else:
             await end_of_interview(message, state, one=1)
        ...
        if amount_questions > 3:
            await state.set_state(SecondInterviewInfoStates.waiting_for_4_question_s)
            await message.reply(f"{json.loads(questions_of_second_interview).get('Вопрос 4')}", reply_markup=cancel)
        else:
             await end_of_interview(message, state, one=1)
    elif await state.get_state() == SecondInterviewInfoStates.waiting_for_4_question_s:
        await state.update_data(waiting_for_4_question_s=message.text)
        if amount_questions > 4:
            await state.set_state(SecondInterviewInfoStates.waiting_for_5_question_s)
            await message.reply(f"{json.loads(questions_of_second_interview).get('Вопрос 5')}", reply_markup=cancel)
        else:
             await end_of_interview(message, state, one=1)
    elif await state.get_state() == SecondInterviewInfoStates.waiting_for_5_question_s:
        await state.update_data(waiting_for_5_question_s=message.text)
        await end_of_interview(message, state, one=1)
async def end_of_interview(message: types.Message, state: FSMContext, one=1):
        data = await state.get_data()
        # questions_and_responses_of_first_interview = {item[0]: item[1] for item in zip(questions.values(), [data[f'waiting_for_{i+1}_question_f'] for i in range(3)])}
        if one:
            questions_and_responses_of_second_interview = {item[0]: item[1] for item in zip(list(json.loads(questions_of_second_interview).values()), [data[f'waiting_for_{i+1}_question_s'] for i in range(amount_questions)])}
            print('\n\n!!!!\n\n questions_and_responses_of_second_interview', questions_and_responses_of_second_interview)
            await state.clear()
            resp = await process_analise_interview(message, json.dumps(questions_and_responses_of_second_interview, ensure_ascii=False))
            # resp = await process_analise_interview(message, json.dumps([questions_and_responses_of_first_interview, questions_and_responses_of_second_interview], ensure_ascii=False))
        else:
            await state.clear()
            # resp = await process_analise_interview(message, json.dumps(questions_and_responses_of_first_interview, ensure_ascii=False))
        if resp.lower() == 'да':
            ...
            if t:=vacancy.test_task:
                await state.update_data(waiting_for_vacancy_test=json.loads(t))
                keyboard = ReplyKeyboardMarkup(
                    keyboard=[
                        [
                ...
                await record_history_by_user_id(message.from_user.id, {'role': 'assistant', 'content': 'Поздравляю вас с успешным прохождением первого этапа отбора! Готовы ли вы выполнить тестовое задание, чтобы пройти в финал и выйти на собеседованием с руководителем отдела?'}, state)
                await message.answer(f"Поздравляю вас с успешным прохождением первого этапа отбора! Готовы ли вы выполнить тестовое задание, чтобы пройти в финал и выйти на собеседованием с руководителем отдела?", reply_markup=keyboard)
            else:
                keyboard = ReplyKeyboardMarkup(
                    keyboard=[
                        [
                ...
                await record_history_by_user_id(message.from_user.id, {'role': 'assistant', 'content': 'Поздравляю вас с успешным прохождением первого этапа отбора! Готовы ли вы к собеседованию с руководителем отдела?'}, state)
                await message.answer(f"Поздравляю вас с успешным прохождением первого этапа отбора! Готовы ли вы к собеседованию с руководителем отдела?", reply_markup=keyboard)
        elif resp.lower() == 'нет':
            await message.answer(f"Кандидат плохо отвечал на вопросы")
        else:
